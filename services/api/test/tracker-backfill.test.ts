...
function makeService(handle: StoreHandle, replies: (string | Error)[]): {
	service: SceneTrackerService;
	contextInputs: Array<{ throughMessageId?: string }>;
} {
	const queue = [...replies];
	const contextInputs: Array<{ throughMessageId?: string }> = [];
	...
	const sessionRuntime = {
		chatLifecycle: {
			buildPipelineContext: async (input: { throughMessageId?: string }) => {
...
describe("SceneTrackerService backfill — manifest freeze + modes (SCN-14)", () => {
	...
	it("binds every manifest item to its own history prefix (throughMessageId)", async () => {
		const handle = makeStore({
			assistants: [
		...
		const { service, contextInputs } = makeService(handle, [VALID_REPLY, VALID_REPLY, VALID_REPLY]);
		const status = await service.startBackfill(CHAT, SCENE_BACKFILL_MODE.rebuild);
		await awaitTerminal(service, status.runId);
		expect(contextInputs.map((input) => input.throughMessageId)).toEqual(["m1", "m2", "m3"]);
	});
