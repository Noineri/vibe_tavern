...
describe("SP-7 preset → service-prompt profile migration", () => {
	...
	test("name collision with the built-in Default → profile named \"Default (copy)\"", async () => {
		const { presets, profileStore, stores } = await setup();
		...
		expect(result.created[0].profileName).toBe("Default (copy)");
		...
		const copy = profiles.find((p) => p.name === "Default (copy)")!;
		expect(builtin.isDefault).toBe(true);
		expect(copy.isDefault).toBe(false);
		expect(copy.overrides.summary).toBe("SUMMARY-OVR");
	});

	test("two presets sharing a name → second gets \"(copy)\", third gets \"(copy) 2\"", async () => {
		const { presets, profileStore, stores } = await setup();
		...
		const result = await migratePresetServicePrompts(stores);
		expect(result.created.map((e) => e.profileName)).toEqual(["Twin", "Twin (copy)", "Twin (copy) 2"]);
		const profiles = await profileStore.listServicePromptProfiles();
		expect(profiles.find((p) => p.name === "Twin")!.overrides.summary).toBe("FIRST");
		expect(profiles.find((p) => p.name === "Twin (copy)")!.overrides.summary).toBe("SECOND");
		expect(profiles.find((p) => p.name === "Twin (copy) 2")!.overrides.summary).toBe("THIRD");
	});
