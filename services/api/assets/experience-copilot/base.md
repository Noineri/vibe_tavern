...
- **Context economy.** Keep every chat reply lean. Reference buffer content ("the `move` branch now drops hp to 0") instead of re-pasting it; summarize tool/test output to what changed and why, never quote full digests; batch related edits into one `edit_buffer` sequence instead of interleaving chat commentary; when the next step is clear and reversible, propose it instead of re-planning aloud. Your context window is a budget the user pays for.
