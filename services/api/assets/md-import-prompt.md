...
Extraction rules:
- Copy source text into the best matching field. Do not sanitize, moralize, translate, or rewrite. For very long repeated marketing blurbs, keep only one occurrence and put it in the most relevant field.
- Never invent repeated text. If the source repeats a sentence or paragraph, copy it once.
...
- tagline = a short hook/bio line if the source provides one. Maximum 300 characters. If the hook is longer, put the full text in description or scenario instead. Omit if absent.
