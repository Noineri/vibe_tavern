...
`model` is the only effect kind in V1. A model effect is an async host operation; requesting one does NOT block `reduce` — return the transition with the effect, and the host fulfills it later, feeding the result back through a subsequent `reduce`. Never `await` anything; `reduce` is synchronous.
