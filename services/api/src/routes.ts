...
export function createApiRouter(
  ...
  api.post("/api/debug/send-log", zValidator("json", schemas.debugSendLogSchema), (c) => {
  ...
  api.get("/api/personas", (c) => {
    return c.json(runtime.listPersonas());
  });

  api.post("/api/personas", zValidator("json", schemas.createPersonaSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.createPersona(body), 201);
  });

  api.get("/api/chats/:chatId", (c) => {
    return c.json(runtime.getChatSnapshot(c.req.param("chatId")));
  });

  api.post("/api/characters", zValidator("json", schemas.createCharacterSchema), (c) => {
    const body = c.req.valid("json");
  ...
  api.post("/api/chats", zValidator("json", schemas.createChatSchema), (c) => {
    const body = c.req.valid("json");
  ...
  api.post("/api/chats/:chatId/clone", (c) => {
    return c.json(runtime.cloneChat(c.req.param("chatId")));
  });

  api.get("/api/characters/:characterId/export", (c) => {
    return c.json(runtime.exportCharacter(c.req.param("characterId")));
  });

  api.get("/api/chats/:chatId/export.jsonl", (c) => {
    return c.text(
  ...
  api.get("/api/prompt-traces/:traceId/export", (c) => {
    return c.json(runtime.exportPromptTrace(c.req.param("traceId")));
  });

  api.patch("/api/chats/:chatId/settings", zValidator("json", schemas.updateChatSettingsSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(
  ...
  api.post("/api/chats/:chatId/messages/:messageId/branch", (c) => {
    return c.json(runtime.branchChat(c.req.param("chatId"), c.req.param("messageId")));
  });

  api.post("/api/chats/:chatId/messages/:messageId/regenerate", async (c) => {
  ...
  api.post("/api/chats/:chatId/messages/:messageId/variants/:variantIndex/select", (c) => {
    return c.json(
  ...
  api.patch("/api/chats/:chatId/messages/:messageId", zValidator("json", schemas.editMessageSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.editMessage(c.req.param("chatId"), c.req.param("messageId"), body.content ?? ""));
  });

  api.delete("/api/chats/:chatId/messages/:messageId", (c) => {
    return c.json(runtime.deleteMessage(c.req.param("chatId"), c.req.param("messageId")));
  });

  api.post("/api/chats/:chatId/messages", zValidator("json", schemas.sendMessageSchema), async (c) => {
    const chatId = c.req.param("chatId");
    const body = c.req.valid("json");
  ...
  api.post("/api/chats/:chatId/set-persona", zValidator("json", schemas.setPersonaSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.setChatPersona(c.req.param("chatId"), body.personaId));
  });

  api.post("/api/chats/:chatId/set-prompt-preset", zValidator("json", schemas.setPromptPresetSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.setChatPromptPreset(c.req.param("chatId"), body.promptPresetId));
  });

  api.post("/api/chats/:chatId/fork", (c) => {
    return c.json(runtime.forkBranch(c.req.param("chatId")));
  });

  api.post("/api/chats/:chatId/branches/:branchId/activate", (c) => {
    return c.json(runtime.activateBranch(c.req.param("chatId"), c.req.param("branchId")));
  });

  api.delete("/api/chats/:chatId/branches/:branchId", (c) => {
    return c.json(runtime.deleteBranch(c.req.param("chatId"), c.req.param("branchId")));
  });

  api.delete("/api/chats/:chatId", (c) => {
    runtime.deleteChat(c.req.param("chatId"));
    return c.body(null, 204);
  });

  api.patch("/api/chats/:chatId/title", zValidator("json", schemas.renameChatSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.renameChat(c.req.param("chatId"), body.title));
  });

  api.patch("/api/characters/:characterId/archive", (c) => {
    return c.json(runtime.archiveCharacter(c.req.param("characterId")));
  });

  api.patch("/api/characters/:characterId/unarchive", (c) => {
    return c.json(runtime.unarchiveCharacter(c.req.param("characterId")));
  });

  api.patch("/api/characters/:characterId", zValidator("json", schemas.updateCharacterSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(
  ...
  api.delete("/api/characters/:characterId", (c) => {
    runtime.deleteCharacter(c.req.param("characterId"));
    return c.body(null, 204);
  });

  api.patch("/api/personas/:personaId", zValidator("json", schemas.updatePersonaSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(
  ...
  api.delete("/api/personas/:personaId", (c) => {
    try {
  ...
  api.get("/api/personas/:personaId/personal-lorebook", (c) => {
    try {
  ...
  api.put("/api/personas/:personaId/personal-lorebook", zValidator("json", schemas.setPersonalLorebookSchema), (c) => {
    const body = c.req.valid("json");
  ...
  api.patch("/api/lorebooks/:lorebookId", zValidator("json", schemas.updateLorebookSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(
  ...
  api.post("/api/lorebooks/:lorebookId/test-activation", zValidator("json", schemas.testActivationSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(
  ...
  api.get("/api/lorebooks/:lorebookId/entries", (c) => {
    return c.json(runtime.listLoreEntries(c.req.param("lorebookId")));
  });

  api.post("/api/lorebooks/:lorebookId/entries", zValidator("json", schemas.createLoreEntrySchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.createLoreEntry(c.req.param("lorebookId"), body));
  });

  api.patch("/api/lorebooks/:lorebookId/entries/:entryId", zValidator("json", schemas.updateLoreEntrySchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.updateLoreEntry(c.req.param("lorebookId"), c.req.param("entryId"), body));
  });

  api.delete("/api/lorebooks/:lorebookId/entries/:entryId", (c) => {
    runtime.deleteLoreEntry(c.req.param("lorebookId"), c.req.param("entryId"));
    return c.json({ ok: true });
  });

  api.get("/api/prompt-presets", (c) => {
    return c.json(runtime.listPromptPresets());
  });

  api.post("/api/prompt-presets", zValidator("json", schemas.createPromptPresetSchema), (c) => {
    const body = c.req.valid("json");
    try {
  ...
  api.patch("/api/prompt-presets/:presetId", zValidator("json", schemas.updatePromptPresetSchema), (c) => {
    const body = c.req.valid("json");
    try {
  ...
  api.delete("/api/prompt-presets/:presetId", (c) => {
    try {
  ...
  api.get("/api/providers/:providerId", (c) => {
    return c.json(runtime.fetchProviderProfile(c.req.param("providerId")));
  });

  api.delete("/api/providers/:providerId", (c) => {
    runtime.deleteProviderProfile(c.req.param("providerId"));
    return c.json({ ok: true });
  });

  api.patch("/api/providers/:providerId", zValidator("json", schemas.updateProviderProfileSchema), async (c) => {
    const body = c.req.valid("json");
    return c.json(await runtime.updateProviderProfile(c.req.param("providerId"), body));
  });

  api.post("/api/providers/test", zValidator("json", schemas.testProviderDraftSchema), async (c) => {
    const body = c.req.valid("json");
    return c.json(await runtime.testProviderDraft(body as any));
  });

  api.post("/api/import/json", zValidator("json", schemas.importJsonSchema), (c) => {
    const body = c.req.valid("json");
    return c.json(runtime.importJson(body));
  });

  api.post("/api/providers", zValidator("json", schemas.saveProviderDraftSchema), async (c) => {
    const body = c.req.valid("json");
    return c.json(await runtime.saveProviderDraft(body));
  });

  api.post("/api/providers/:providerId/activate", (c) => {
    return c.json(runtime.activateProviderProfile(c.req.param("providerId")));
  });

  api.post("/api/providers/fetch-models", zValidator("json", schemas.fetchModelsSchema), async (c) => {
    const body = c.req.valid("json");
  ...
  api.post("/api/providers/test-chat", zValidator("json", schemas.testChatSchema), async (c) => {
    const body = c.req.valid("json");
    const baseUrl = body?.baseUrl ?? "";
    const apiKey = body?.apiKey ?? "";
  ...
  api.post("/api/providers/:providerId/models", (c) => {
    return c.json(runtime.fetchProviderModels(c.req.param("providerId")));
  });

  api.post("/api/providers/:providerId/test", (c) => {
    return c.json(runtime.testProviderProfile(c.req.param("providerId")));
  });

  api.post("/api/providers/:providerId/test-chat", zValidator("json", schemas.testChatProfileSchema), async (c) => {
    const body = c.req.valid("json");
    const model = body.model;
    if (!model) {
