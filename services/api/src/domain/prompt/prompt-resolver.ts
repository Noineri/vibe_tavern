...
export class StaticPromptResolver implements PromptAssemblyResolver {
	...
	async getPersona(personaId: string): Promise<PersonaRecord | null> {
		...
		if (!p) return null;
		return { id: p.id, name: p.name, description: p.description, pronouns: p.pronouns, avatarAssetId: p.avatarAssetId, avatarFullAssetId: p.avatarFullAssetId, avatarCropJson: p.avatarCropJson, avatarExt: p.avatarExt, avatarFullExt: p.avatarFullExt, defaultForNewChats: p.defaultForNewChats, avatarDescription: p.avatarDescription, includeAvatarInPrompt: p.includeAvatarInPrompt, updatedAt: p.updatedAt };
	}
