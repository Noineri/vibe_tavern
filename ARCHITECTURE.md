...
3. `baselineLegacyDb()` — if DB has tables but no `__drizzle_migrations` tracking table (pre-migration DB), stamp all existing migrations as already applied
4. `migrate()` — apply any unapplied `.sql` files from `drizzle/`

New migrations are generated by editing `db-schema.ts` then running `bun run db:generate` from repo root.
