...
| **TypeScript 7.1** | Language | Strict mode throughout. Branded IDs (`Brand<"ChatId">`) prevent accidental ID swaps at compile time. All API contracts are Zod schemas that produce TS types. Pinned to one exact 7.1 prerelease build (7.1 is not on `latest` yet — that is still 7.0.2): `bun-types` types `with { type: "text" }` import attributes only from 7.1 on, and the web app relies on them. Relax the pin to a caret range once 7.1 ships stable. |
| **Bun.build + Bun HTML imports** | Frontend build, dev server, HMR | The same runtime bundles the SPA (`scripts/build-web.ts`, HTML entry) and serves it in dev (`apps/web/dev-server.ts` with HMR). Tailwind 4 via `bun-plugin-tailwind`; dev-only `data-component` injection and build-time constants via local Bun plugins. Text imports use Bun's native `type: "text"` attribute, not a custom loader. No separate frontend toolchain. |
