...
**Prior-summaries context (continuity-aware summaries).** When a ranged or auto summary is generated and preceding summaries exist, a `prior_summaries_context` layer (priority 490, between `summary_memory` at 500 and `prompt_preset_summary` at 350) is emitted, carrying the recent preceding summaries as read-only continuity so the new summary does not cold-start. Chain selection lives in `assembleRangedSummaryPrompt` (`session-runtime-chat-lifecycle.ts`): it loads `listByChatBranch`, keeps only summaries whose `summarizedTo` is strictly less than the range's `from` (no overlap with the range being summarized), takes the `maxPriorSummaries` most-recent, and reverses to oldest→newest. The layer is gated on non-empty `priorSummaries` AND `config.summary`, so it never appears on a chat turn. The toggle (`includePriorSummaries`, default ON) and the count cap (`maxPriorSummaries`, default 10, 0–100) are user-facing fields on `autoSummaryConfig` and on the ranged-generate request, surfaced in SummaryTab for both the auto-summary config and the manual ranged path. The full `summarizeChat` path (one growing record) deliberately does NOT load priors — it re-summarizes from scratch, so prior-context would compound drift; only the incremental ranged/auto paths use it.

**Default summary prompt fallback.** An empty `preset.summary` used to produce an instruction-less summary call. `resolveSummaryPrompt` (`domain/prompt/summary-prompt.ts`) now returns the trimmed preset text when present, else loads the bundled `services/api/assets/summary-ai-prompt.md` — authored for the prior-context strategy (read-only continuity framing, what to capture, language-neutral). Wired in `StaticPromptResolver.getPromptPreset` as `summary: await resolveSummaryPrompt(preset.summaryPrompt)`. Presets that carry a `summary_prompt` are byte-equivalent; the fallback only fires on empty.
