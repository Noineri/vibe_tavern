...
async function main() {
	...
	for (let i = 1; i <= N; i++) {
		...
		await writeFile(join(OUT, "characters", `Mock${i}.json`), JSON.stringify(card));
		...
		await writeFile(join(chatDir, `chat-${i}.jsonl`), lines.join("\n"));
	}

	for (let i = 0; i < 3; i++) {
		await writeFile(join(OUT, "worlds", `World${i}.json`), JSON.stringify({ name: `World ${i}`, entries: { "0": { uid: 0, key: ["x"], keysecondary: [], content: "entry", comment: "", constant: false, vectorized: false, selective: true, selectiveLogic: 0, addMemo: false, order: 100, position: 0, disable: false, excludeRecursion: false, preventRecursion: false, delayRecursion: false, probability: 100, useProbability: true, depth: 4, group: "", groupOverride: false, groupWeight: 100, scanDepth: null, caseSensitive: null, matchWholeWords: null, useGroupScoring: null, automationId: "", role: null, sticky: null, cooldown: null, delay: null, displayIndex: i } } }));
	}

	for (let i = 0; i < 2; i++) {
		await writeFile(join(OUT, "OpenAI Settings", `Preset${i}.json`), JSON.stringify({ chat_start: "", prompts: [{ identifier: "main", name: "main", content: "You are a roleplay assistant.", role: "system" }], prompt_order: { dummy: { order: [{ identifier: "main" }] } }, temperature: 0.8 }));
	}

	await writeFile(join(OUT, "settings.json"), JSON.stringify({ persona_descriptions: { "default.png": { description: "A smoke-test persona." } }, default_persona: "default.png" }));
