 ...
 * canonical data), the chat's image-gen profile pick (IG-2 deliberately kept
 * this a chat-level choice), and the six generation-mode recipes.
 *
...
function ImageGenMenuBody({ chatId, messageId, onDone }: {
  ...
  return (
    <div className="flex flex-col gap-2 p-1">
      ...
      {/* Profile pick — feeds the generate call's profileId (IG-17's chip
          reads/writes the same store map). */}
      <div className="flex flex-col gap-1.5 px-1.5">
        ...
        <DropdownSelect
          ...
          triggerTestId="image-gen-profile-select"
          disabled={busy}
        />
      </div>
      ...
      {MODES.map((mode) => {
        ...
        return (
          <button
            ...
            className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-md px-2.5 py-1.5 text-left font-ui text-[calc(var(--ui-fs)-1px)] text-t2 transition-colors hover:bg-s2 hover:text-t1 disabled:cursor-default disabled:opacity-40"
