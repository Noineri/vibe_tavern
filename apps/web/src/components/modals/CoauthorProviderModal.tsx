    ...
    detailContent={!selectedProfile ? <div className="flex h-full items-center justify-center font-ui text-[13px] text-t3">{profiles.length === 0 ? t("coauthor.provider.no_profiles") : t("coauthor.provider.select_profile")}</div> : <div className="flex h-full flex-col gap-4">
      ...
      <div className="rounded-lg border border-border bg-s2 px-4 py-3">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div><label className="mb-1 block font-ui text-[11px] text-t3">{t("coauthor.provider.max_tokens")}</label><NumberInput key={`max-${selectedProfile.id}-${tokenInputRevision}`} value={coauthorMaxTokens ?? inheritedMaxTokens} min={1} max={1_000_000} step={100} disabled={tokenSaving} onChange={(value) => void handleTokenOverride({ coauthorMaxTokens: value })} /><button type="button" disabled={tokenSaving || coauthorMaxTokens === null} onClick={() => void handleTokenOverride({ coauthorMaxTokens: null })} className="mt-1 font-ui text-[10px] text-t4 hover:text-t2 disabled:opacity-50">{t("reset")}</button></div>
