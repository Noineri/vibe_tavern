...
export function ProviderModelList({
  ...
  const renderRow = (model: ProviderModelListOption) => {
  ...
  // Group by owner when enabled AND there is more than one owner — a single
  // owner (e.g. all "Other" on a custom endpoint) collapses back to flat so we
  // don't render a lone redundant header. Items keep their favorites-first /
  // label sort WITHIN each group (visible is already sorted before grouping).
  let groups: Array<[string, ProviderModelListOption[]]> | null = null;
  if (groupByOwner) {
    const map = new Map<string, ProviderModelListOption[]>();
    for (const m of visible) {
      const owner = deriveOwner(m);
      const arr = map.get(owner);
      if (arr) arr.push(m); else map.set(owner, [m]);
    }
    const entries = [...map.entries()].sort(([a], [b]) => a.localeCompare(b));
    if (entries.length > 1) groups = entries;
  }
    ...
    <Command.List className={cn("max-h-[200px] overflow-y-auto bg-surface p-1", listClassName)}>
      {groups
        ? groups.map(([owner, items]) => (
            <Command.Group
              key={owner}
              heading={<div className="px-2.5 pb-1 pt-1.5 font-ui text-[10px] font-semibold uppercase tracking-[0.06em] text-t4">{owner}</div>}
            >
              {items.map(renderRow)}
            </Command.Group>
          ))
