...
/**
 * STT model picker (P8) — VERBATIM VISUAL FORK of TtsModelPicker (itself a
 * fork of ProviderModelSelector; owner directive: the LLM picker design is
 * the reference, no hand-rolled patterns). Owner 2026-09-04: a fetched
 * picker for EVERY backend that can list models — openai-compat
 * (`/models?output_modalities=transcription`) and gemini (the
 * `/v1beta/models` catalogue, non-chat families filtered server-side);
 * whisper-browser keeps its fixed LOCAL roster dropdown (no fetch — the
 * roster renders elsewhere). Rendered BARE (no section card) as the FIRST
 * level-2 section under the base card; a hand-typed id stays reachable
 * through the popover's custom-slug row (the manual-input fallback).
 */

const labelCls = "block text-[calc(var(--ui-fs)-3px)] font-medium tracking-[0.06em] uppercase text-t3";

/** Row option for the STT model list. `id` is the wire model id; `label`
...
export function SttModelPicker({
  ...
  const selectModel = (model: SttModelOption) => {
    onChange(model.id);
    setOpen(false);
    setSearch("");
  };
