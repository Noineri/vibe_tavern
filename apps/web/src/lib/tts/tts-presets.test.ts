...
describe("tts-presets", () => {
  test("has exactly 10 entries with unique ids", () => {
    expect(TTS_PRESETS.length).toBe(10);
    const ids = TTS_PRESETS.map((p) => p.id);
    expect(new Set(ids).size).toBe(10);
    expect(ids).toEqual(["openai", "openrouter", "groq", "siliconflow", "nanogpt", "electronhub", "gemini", "elevenlabs", "cartesia", "inworld"]);
  });
  ...
  test("group-filtering helper behaves like provider-presets counterpart", () => {
    ...
    expect(visibleWithFlag.length).toBe(10);
    expect(getVisibleProviderPresets(false).length).toBe(10);
